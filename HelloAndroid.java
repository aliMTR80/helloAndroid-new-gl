...
import static com.novinsadr.myapplication.BluetoothHelper.BLUETOOTH_PERMISSION_REQUEST_CODE;
...
public class HelloAndroid extends Activity implements Handler.Callback, SensorEventListener {
    ...
    // در بالای کلاس HelloAndroid به عنوان متغیرهای static
...
//                    }
    ...
    private void InitBluetooth() {
        ...
        mBluetoothAdapter.setName("OKM-Rover-UC");
        if (!BluetoothAdapter.checkBluetoothAddress(mSettingsData.BluetoothAddress)) {
            mSettingsData.BluetoothAddress = "00:00:00:00:00:00";
        }

        if (!mSettingsData.BluetoothAddress.equals("00:00:00:00:00:00")) {
            mBluetoothDevice = mBluetoothAdapter.getRemoteDevice(mSettingsData.BluetoothAddress);
            mmBluetoothAddresses.add(mSettingsData.BluetoothAddress);
        }
    ...
    private void setupBluetoothConnection() {
        mBluetoothAdapter.setName(BT_DEVICE_NAME);
    ...
    private void startBluetoothConnection() {
        ...
        // اگر ConnectThread قبلی وجود دارد، آن را پاک می‌کنیم
        if (mConnectThread != null) {
            mConnectThread.stop();
            mConnectThread = null;
        }

        // ایجاد ConnectThread جدید
        mConnectThread = new ConnectThread(mBluetoothDevice, this, mHandler, mmBluetoothAddresses);

        if (mConnectThread != null) {
            // تنظیم ScanDataObject
            mConnectThread.setScanDataObject(mScanDataObject);
            mConnectThread.setDiscovery(true);

            // شروع thread
            mConnectThread.start();

            // نمایش دیالوگ در حال اتصال
            mHandler.sendEmptyMessage(MSG_SHOW_DIALOG_BLUETOOTH_CONNECTING);
        } else {
            // در صورت خطا در ایجاد thread
            Toast.makeText(this, R.string.Bluetooth_ConnectionFailed, Toast.LENGTH_SHORT).show();
            mHandler.sendEmptyMessage(MSG_SHOW_DIALOG_BLUETOOTH_FAILED);
        }
    }

    // متد cancel در ConnectThread
    public void cancel() {
        try {
    ...
    private void showEnableBluetoothDialog() {
        new AlertDialog.Builder(this)
                .setTitle(R.string.MessageBox_Bluetooth)
                .setMessage(R.string.Bluetooth_TurnOn)
                ...
                .setNegativeButton(R.string.Button_No, (dialog, which) -> onBackPressed())
                .setCancelable(false)
                .show();
    }
